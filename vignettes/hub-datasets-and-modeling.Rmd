...
Results can be sorted by `"downloads"`, `"likes"`, `"created"`, or `"updated"`,
...
- **API rate limits.** `bake()` makes one API call per text row. For large
  datasets, consider pre-computing embeddings with `hf_embed()` and saving
  them to disk with `saveRDS()`.
