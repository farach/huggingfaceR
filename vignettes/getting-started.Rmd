...
2. Follow the [Hugging Face access tokens documentation](https://huggingface.co/docs/hub/security-tokens).
3. Generate a token with the **Make calls to Inference Providers** permission.
   A read-only token is enough for Hub browsing, but inference requests are
   rejected without the Inference Providers permission.
...
# Store your token persistently (writes HF_TOKEN to .Renviron)
...
huggingfaceR reads `HF_TOKEN` and falls back to the legacy
`HUGGING_FACE_HUB_TOKEN`, so tokens configured for the `hf` CLI or the Python
client are picked up automatically.

If you belong to a Team or Enterprise organization and want usage billed to the
organization rather than to you personally, set `HF_BILL_TO`:

```{r bill-to, eval = FALSE}
